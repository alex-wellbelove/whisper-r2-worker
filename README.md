...
## R2/S3 Output Storage (Fork Addition)

This fork adds support for uploading large transcription results to R2/S3-compatible storage, solving the RunPod API response size limit issue for long audio files.
...
|----------|-------------|
| `R2_ENDPOINT_URL` | R2/S3 endpoint URL (e.g., `https://xxx.r2.cloudflarestorage.com`) |
| `R2_ACCESS_KEY_ID` | Access key ID |
| `R2_SECRET_ACCESS_KEY` | Secret access key |
| `R2_BUCKET_NAME` | Bucket name |
| `R2_PUBLIC_URL` | Public URL prefix (e.g., `https://pub-xxx.r2.dev`) |

### Configuration via Request

Alternatively, pass `s3Config` in your request input:
...
When R2 output is configured, the response will be:

```json
{
  "status": "completed",
  "result_url": "https://pub-xxx.r2.dev/transcriptions/20260206_123456_jobid.json",
...
The full transcription (with segments, timestamps, etc.) is available at `result_url`.
